...
class ReposTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        fetchRepos(forUsername: "APP-iOS5th"){ [weak self] result in switch result {
        case .success(let repos):
            self?.repos = repos
        case .failure(let error):
            self?.repos = []
            print("There was ans error: \(error)")
        }
    ...
    override func numberOfSections(in tableView: UITableView) -> Int {
        // #warning Incomplete implementation, return the number of sections1
        return 1
    }
...
extension ReposTableViewController: UITextFieldDelegate {
    public func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        guard let enterdUsername = textField.text else {
            repos = []
            tableView.reloadData()
            return true
        }
        
        fetchRepos(forUsername: enterdUsername) {
            [weak self] result in
            switch result {
            case .success(let repos):
                self?.repos = repos
