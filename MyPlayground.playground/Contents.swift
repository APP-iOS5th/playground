...
// 배열 => 연속된 메모리 영역에 순서대로 저장됨
var moviesToWatch: Array<String> = Array() //array도 클래스고 객체이다 init 해줌 array()는 붕어빵 틀 배열에 대한 클래스 인스턴스 만들어짐 그게 moviesToWatch
// var moviesToWatch: [String] = Array() 로도 표현할 수 잇음
...
moviesToWatch.insert("The Matrix", at: 2) //배열 인서트는 비효율적 계속 밀어내고 하나넣고 해서
print(moviesToWatch.first ?? "Empty") //왜 옵셔널이냐? 배열이 비어있을수도 있어서..? 리턴타입이 옵셔널,,
...
let spyMovieSuggestions: [String] = ["The Bourne Identity", "Casino Royale", "Mission Impossioble"]
moviesToWatch = moviesToWatch + spyMovieSuggestions //덧셈 순서대로 앞에 배열 깔고 뒤에 배열
print(moviesToWatch)
print(moviesToWatch.count)
...
moviesToWatch.remove(at: 6) //지운 다음 리턴?

print(moviesToWatch.count)

//배열은 값 타입이지 레퍼런스가 아님 그냥 그 자리에 있음. 수정하면 복사본이 생김 하나만 붙여지는게 아니고,, 불변성(그 자리에 안바뀌는게 얻는게 더 많음..)
