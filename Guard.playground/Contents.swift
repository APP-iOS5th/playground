...
func makePlanet(fromInput input: [String: Any]) -> Planet? {
    guard let name = input["name"] as? String, let positionFromSun = input["positionFromSun"] as? Int, let fractionOfEarthMass = input["fractionOfEarthMass"] as? Double, let distanceFromSunInAus = input["distanceFromSunInAus"] as? Double, let hasRings = input["hasRing"] as? Bool else {
        return nil
    }
